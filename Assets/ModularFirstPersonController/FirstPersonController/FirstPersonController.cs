...
//
// "Added second camera and V key switching between cameras" || version 1.0.2
...
public class FirstPersonController : MonoBehaviour
{
    ...
    public Camera secondCamera; // Добавлена вторая камера
    private bool useMainCamera = true; // Флаг для переключения между камерами
    ...
    private void Awake()
    {
        ...
        if (secondCamera != null) secondCamera.fieldOfView = fov;
        originalScale = transform.localScale;
        jointOriginalPos = joint.localPosition;

        // Деактивируем вторую камеру при старте
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.V)) 
        {
            ...
            if (playerCamera != null) playerCamera.gameObject.SetActive(useMainCamera);
            if (secondCamera != null) secondCamera.gameObject.SetActive(!useMainCamera);
        }
        #endregion
        ...
        if (cameraCanMove)
        {
            ...
            // Применяем поворот к активной камере
            if (useMainCamera && playerCamera != null)
                playerCamera.transform.localEulerAngles = new Vector3(pitch, 0, 0);
            else if (!useMainCamera && secondCamera != null)
                secondCamera.transform.localEulerAngles = new Vector3(pitch, 0, 0);
        }
        ...
        if (enableZoom)
        {
            ...
            Camera activeCamera = useMainCamera ? playerCamera : secondCamera;
            if (activeCamera != null)
            {
                if (isZoomed)
                {
                    activeCamera.fieldOfView = Mathf.Lerp(activeCamera.fieldOfView, zoomFOV, zoomStepTime * Time.deltaTime);
                }
                else if (!isZoomed && !isSprinting)
                {
                    activeCamera.fieldOfView = Mathf.Lerp(activeCamera.fieldOfView, fov, zoomStepTime * Time.deltaTime);
                }
            }
        }
        ...
        if (enableSprint)
        {
            if (isSprinting)
            {
                ...
                if (activeCamera != null)
                {
                    activeCamera.fieldOfView = Mathf.Lerp(activeCamera.fieldOfView, sprintFOV, sprintFOVStepTime * Time.deltaTime);
                }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.playerCamera = (Camera)EditorGUILayout.ObjectField(new GUIContent("Main Camera", "Main camera attached to the controller."), fpc.playerCamera, typeof(Camera), true);
        fpc.secondCamera = (Camera)EditorGUILayout.ObjectField(new GUIContent("Second Camera", "Secondary camera that can be switched to with V key."), fpc.secondCamera, typeof(Camera), true);
